...
def q1():
    diff_quantis = []
    for i in list([0.25, 0.5, 0.75]):
        sub_stat = round((dataframe.normal.quantile(i)) - (dataframe.binomial.quantile(i)),3)
        diff_quantis.append(sub_stat)
    return tuple(diff_quantis)
...
def q3():
    diff = dataframe.binomial.mean() - dataframe.normal.mean(), np.var(dataframe.binomial) - np.var(dataframe.normal)
    return tuple([round(x,3) if isinstance(x, float) else x for x in diff])
